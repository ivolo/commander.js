...
/**
 * Checks for missing mandatory options.
 *
 * @api private
 */
Command.prototype.checkMandatoryOptiuons = function () {
  // check for mandatory arguments that were not provided
  for (var i = 0; i < this.options.length; i++) {
    var option = this.options[i];
    if (option.mandatory && undefined === this[option.name()]) {
      this.missingMandatoryOption(option);
    }
  }
};
...
/**
 * Mandatory option `name` is missing.
 *
 * @param {String} option
 * @api private
 */

Command.prototype.missingMandatoryOption = function(option){
  console.error();
  console.error("  error: missing mandatory option `%s'", option.name());
  console.error();
  process.exit(1);
};
